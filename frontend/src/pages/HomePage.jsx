...
import OrganikPage from './OrganikPage.jsx';
import AnorganikPage from './AnorganikPage.jsx';
...
const HomePage = () => {
    ...
    useEffect(() => {
        const userString = localStorage.getItem("user"); // Ambil string JSON
        if (userString) {
            const userObj = JSON.parse(userString); // Ubah ke object JS
            setUserLocal(userObj);
            setNameLocal(userObj.name); // Ambil nama
        }
    ...
    return (
        <MainLayout notifSlug={notifSlug}>
            {guidePopup === "organic" ? (
                <OrganikPage setMenu={(v) => setGuidePopup(v)} />
            ) : guidePopup === "anorganic" ? (
                <AnorganikPage setMenu={(v) => setGuidePopup(v)} />
            ) : (
                ...
                <div className='bg-[#f8f8f8]'>
                    <main className='flex flex-col items-center justify-start px-6'>
                        <container className='flex max-w-7xl w-full flex-col items-start justify-center gap-10 py-12'>
                            <div>
                                <h3 className='text-[20px] font-semibold'>Hello, <span className='text-primary'>{user ? (user.name) : nameLocal ? nameLocal : "..."}</span></h3>
                            ...
                            <div className='flex flex-wrap justify-between items-start w-full gap-4'>
                                <div className='flex flex-col max-w-[560px] justify-start items-start w-full gap-4'>
                                    ...
                                    <div className='min-w-full flex gap-10 max-lg:gap-8 max-md:gap-6'>
                                        ...
                                        <button onClick={() => { setGuidePopup("organic") }} className='flex cursor-pointer flex-col items-center justify-center gap-2 outline-1 w-full py-6 max-md:py-4 outline-[#EBEBEB] rounded-[20px] bg-white p-6'>
                                            <img src={OrganicIcon} className='h-[45px] w-[45px] max-md:w-[30px] max-md:h-[30px]' alt="" />
                                            <p className='max-md:text-base'>Organik</p>
                                        </button>
                                        <button onClick={() => { setGuidePopup("anorganic") }} className='flex cursor-pointer flex-col items-center justify-center gap-2 outline-1 w-full py-6 max-md:py-4 outline-[#EBEBEB] rounded-[20px] bg-white p-6'>
                                    ...
                                    <div className='flex flex-col gap-2 w-full py-6'>
                                        <div className='flex w-full gap-6 max-md:gap-2 border-b border-[#E8E8E8] max-md:text-base'>
                                            ...
                                            <div className='flex flex-col w-full items-end justify-center gap-2 pb-6 max-md:pb-2'>
                                                <p className='text-grey-300'>11 April</p>
                                                <div className='bg-green p-1 max-md:py-0.5 px-2 rounded-[8px] max-md:rounded-[6px] max-md:text-[8px] text-white'>Complete</div>
                                            </div>
                                        </div>
