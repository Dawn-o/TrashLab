...
const GuideSection = ({ title, icon, children }) => (
  <div className='w-[450px] flex flex-col justify-start items-start gap-6'>
                ...
                <GuideSection title="Yang Bisa Kamu Lakukan" icon={Check}>
                    <div className='w-full flex flex-col justify-start items-start gap-4'>
                        {currentGuides.do.map((guide, index) => (
                            <div key={guide.id} className='w-full flex flex-col justify-start items-start'>
                                <p className='text-[16px] font-semibold flex'>
                                ...
                                {guide.points && (
                                    <div className="flex flex-col justify-start items-start w-full">
                                        {guide.points.map((point, idx) => (
                                            <p key={idx} className="text-[12px] w-full">
                    ...
                    <div className='w-full flex flex-col justify-start items-start gap-4'>
                        {Array.isArray(currentGuides.dont) ? (
                            currentGuides.dont.map((guide, index) => (
                                <p key={index} className='text-[12px] font-medium flex'>
                                    <span className="min-w-[18px]">{index + 1}.</span> {guide}
                                </p>
                            ))
                        ) : (
                            <p className='text-[12px] font-medium'>{currentGuides.dont}</p>
                        )}
                    </div>
                </GuideSection>
            </div>
