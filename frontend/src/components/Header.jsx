      ...
      <div className="flex justify-between max-md:justify-end items-center max-w-7xl w-full px-2 max-md:relative">
          ...
          <ul className="flex gap-6 items-center">
            {navItems.map((item, index) => {
              return (
                <li
                  key={index}
                  className={`font-medium text-[18px] px-2 py-4 max-md:text-[12px] rounded-[8px] max-md:rounded-[5px]  ${
                    activeTab === index ? "text-white bg-green" : "text-black"
                  }`}
                >
                  <a
        ...
        <div className="flex gap-12 max-md:gap-8 max-sm:gap-4 items-center ">
          <div
            ...
            className={`font-medium max-md:text-[9px] flex justify-center items-center ${
              !user ? "cursor-pointer" : ""
          ...
          <div className="relative group">
            ...
            <div className="flex flex-col w-[150px] text-[#1E1E1E] font-medium absolute top-16 right-0 shadow-lg z-10 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-[700ms] ease-out">
              {!user ? (
                ...
                <>
                  <button className="rounded-t-[8px] w-full cursor-pointer text-center py-4 px-6 outline-2 outline-[#ECECEC] bg-white hover:bg-gray-100">
                    Profile saya
                  </button>
                  <button
